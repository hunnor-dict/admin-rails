...
class Entry
		...
		if @id == "N"
			...
			forms_new.each do |par_key, par_val|
				par_val.each do |seq_key, seq_val|
					sql.push "INSERT INTO #{tables[:forms]} (#{columns[:forms][:id]}, #{columns[:forms][:entry]}, #{columns[:forms][:orth]}, #{columns[:forms][:pos]}, #{columns[:forms][:par]}, #{columns[:forms][:seq]}, #{columns[:forms][:status]}) VALUES('#{@id}', '#{@entry}', '#{seq_val}', '#{values[:pos]}', '#{par_key}', '#{seq_key}', '#{values[:status]}')"
				end
			end
		else
			action = "update"
			...
			if @pos != values[:pos]
				form_global_changes.push "#{columns[:forms][:pos]} = '#{values[:pos]}'"
				@pos = values[:pos]
			end
			...
			removed_forms.each do |removed_form|
				...
				sql.push "DELETE FROM #{tables[:forms]} WHERE #{columns[:forms][:id]} = '#{@id}' AND #{columns[:forms][:par]} = '#{rm_a[0]}' AND #{columns[:forms][:seq]} = '#{rm_a[1]}' AND #{columns[:forms][:orth]} = '#{rm_a[2]}'"
			end
			added_forms.each do |added_form|
				add_a = added_form.split ";"
				sql.push "INSERT INTO #{tables[:forms]} (#{columns[:forms][:id]}, #{columns[:forms][:entry]}, #{columns[:forms][:orth]}, #{columns[:forms][:pos]}, #{columns[:forms][:par]}, #{columns[:forms][:seq]}, #{columns[:forms][:status]}) VALUES ('#{@id}', '#{@entry}', '#{add_a[2]}', '#{values[:pos]}', '#{add_a[0]}', '#{add_a[1]}', '#{values[:status]}')"
			end
		end
			...
			if values[:trans] != @trans
				if action == "update"
					...
					if trans_check_res.size == 0
						sql.push "INSERT INTO #{tables[:trans]} (#{columns[:trans][:id]}, #{columns[:trans][:trans]}) VALUES ('#{@id}', '#{values[:trans]}')"
					else
						sql.push "UPDATE #{tables[:trans]} SET #{columns[:trans][:trans]} = '#{values[:trans]}' WHERE #{columns[:trans][:id]} = '#{id}'"
					end
				else
					sql.push "INSERT INTO #{tables[:trans]} (#{columns[:trans][:id]}, #{columns[:trans][:trans]}) VALUES ('#{@id}', '#{values[:trans]}')"
				end
	...
	def delete
		...
		columns = database.columns
		sql = []
		sql.push "DELETE FROM #{tables[:forms]} WHERE #{columns[:forms][:id]} = '#{@id}'"
		sql.push "DELETE FROM #{tables[:trans]} WHERE #{columns[:forms][:id]} = '#{@id}'"
		sql.each do |sql_query|
			db.query sql_query
		end
		database.close
